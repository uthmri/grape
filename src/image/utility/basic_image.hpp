...
public:
    pixel_type at(long x) const //08/08/2017  Xiaojun
    {
        return data[x];
    }
    reference at(long x)//08/08/2017  Xiaojun
    {
        return data[x];
    }
...
public:
    ...
    void save_to_ana_file(const char* file_name, const char* hfile_name) const
    {
        format_type out;
        out.load_from_image(*this);
        out.save_to_ana_file(file_name, hfile_name);
    }
    template<typename format_type>
    void set_slope_intercept(const float s, const float c)
    {
        format_type out;
        out.set_slope_intercept(s, c);
    }
    template<typename format_type>
    void set_voxel_size1(const float s1, const float s2, const float s3)
    {
        format_type out;
        out.set_voxel_size1(s1,s2,s3);
    }
